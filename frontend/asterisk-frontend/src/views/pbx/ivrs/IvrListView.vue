...
<template>
  <PageHeader title="Sesli yanıt (IVR)" :description="config.description">
    <button class="button" :disabled="loading" @click="initialize"><AppIcon name="refresh" :size="16" />Yenile</button>
    <RouterLink :to="url(`${basePath}/create`)" class="button button-primary"><AppIcon name="plus" :size="16" />IVR oluştur</RouterLink>
  </PageHeader>
  ...
  <section class="panel">
    ...
    <EmptyState
      ...
      :description="error ? 'Bağlantınızı kontrol edip yeniden deneyin.' : search ? 'Arama yalnızca açık sayfadaki kayıtları kapsar.' : 'İlk ıvr kaydınızı oluşturabilirsiniz.'"
    />
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in visibleRows" :key="String(row.id)">
            <td><RecordIdentity :name="row.name" :id="row.id" icon="mic" /></td>
            <td>{{ displayValue(row.audioFile, 'audioFile') }}</td>
            ...
            <td><div class="actions">
              <RouterLink :to="url(`${basePath}/${row.id}/options`)" class="button button-small">Seçenekler</RouterLink>
