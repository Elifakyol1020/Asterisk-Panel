...
<template>
  <PageHeader title="Kuyruk üyeleri" :description="config.description">
    <template #breadcrumb>
  ...
  <section class="panel">
    ...
    <EmptyState
      ...
      :description="error ? 'Bağlantınızı kontrol edip yeniden deneyin.' : search ? 'Arama yalnızca açık sayfadaki kayıtları kapsar.' : 'İlk kuyruk üyesi kaydınızı oluşturabilirsiniz.'"
    />
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in visibleRows" :key="String(row.id)">
            <td><RecordIdentity :name="row.endpointId" :id="row.id" icon="users" /></td>
  ...
  <ConfirmDialog :open="Boolean(deleteTarget)" title="Kaydı sil" confirm-label="Evet, sil" :busy="deleting" @cancel="deleteTarget = null" @confirm="remove">
    <strong>{{ deleteTarget?.endpointId }}</strong> kalıcı olarak silinecek. Bu işlem geri alınamaz.
  </ConfirmDialog>
</template>
