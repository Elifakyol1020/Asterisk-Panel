...
import { inboundRouteResource as config } from '@/config/resources/inbound-routes'
...
<template>
  <PageHeader title="Gelen arama kuralları" :description="config.description">
    <button class="button" :disabled="loading" @click="initialize"><AppIcon name="refresh" :size="16" />Yenile</button>
    <RouterLink :to="url(`${basePath}/create`)" class="button button-primary"><AppIcon name="plus" :size="16" />Gelen arama kuralı oluştur</RouterLink>
  </PageHeader>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in visibleRows" :key="String(row.id)">
            ...
            <td>{{ displayValue(row.did, 'did') }}</td>
            <td>#{{ row.trunkId }}</td>
            <td>{{ displayValue(row.targetType, 'targetType') }} · #{{ row.targetId }}</td>
