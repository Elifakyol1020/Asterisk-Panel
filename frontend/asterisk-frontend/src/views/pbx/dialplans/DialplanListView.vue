...
<template>
  <PageHeader title="Arama planları" :description="config.description">
    <button class="button" :disabled="loading" @click="initialize"><AppIcon name="refresh" :size="16" />Yenile</button>
    <RouterLink :to="url(`${basePath}/create`)" class="button button-primary"><AppIcon name="plus" :size="16" />Arama planı oluştur</RouterLink>
  </PageHeader>
  ...
  <section class="panel">
    ...
    <EmptyState
      ...
      :description="error ? 'Bağlantınızı kontrol edip yeniden deneyin.' : search ? 'Arama yalnızca açık sayfadaki kayıtları kapsar.' : 'İlk arama planı kaydınızı oluşturabilirsiniz.'"
    />
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in visibleRows" :key="String(row.id)">
            <td><RecordIdentity :name="row.extension" :id="row.id" icon="route" /></td>
            <td>{{ displayValue(row.priority, 'priority') }}</td>
            <td>{{ displayValue(row.application, 'application') }}</td>
            <td><StatusBadge :value="row.enabled" field="enabled" /></td>
            <td v-if="auth.isSuperAdmin">#{{ row.tenantId }}</td>
            <td><div class="actions">
              <RouterLink :to="url(`${basePath}/${row.id}/edit`)" class="icon-button" :aria-label="`${row.extension} düzenle`"><AppIcon name="edit" :size="17" /></RouterLink>
              <button class="icon-button" :aria-label="`${row.extension} sil`" @click="deleteTarget = row"><AppIcon name="trash" :size="17" /></button>
            </div></td>
          </tr>
        </tbody>
      </table>
  ...
  <ConfirmDialog :open="Boolean(deleteTarget)" title="Kaydı sil" confirm-label="Evet, sil" :busy="deleting" @cancel="deleteTarget = null" @confirm="remove">
    <strong>{{ deleteTarget?.extension }}</strong> kalıcı olarak silinecek. Bu işlem geri alınamaz.
  </ConfirmDialog>
</template>
