...
<template>
  <PageHeader title="Trunk’lar" :description="config.description">
    <button class="button" :disabled="loading" @click="initialize"><AppIcon name="refresh" :size="16" />Yenile</button>
    <RouterLink :to="url(`${basePath}/create`)" class="button button-primary"><AppIcon name="plus" :size="16" />Trunk oluştur</RouterLink>
  </PageHeader>
  ...
  <section class="panel">
    ...
    <EmptyState
      ...
      :description="error ? 'Bağlantınızı kontrol edip yeniden deneyin.' : search ? 'Arama yalnızca açık sayfadaki kayıtları kapsar.' : 'İlk trunk kaydınızı oluşturabilirsiniz.'"
    />
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in visibleRows" :key="String(row.id)">
            <td><RecordIdentity :name="row.name" :id="row.id" icon="network" /></td>
            <td>{{ displayValue(row.host, 'host') }}</td>
            <td>{{ displayValue(row.port, 'port') }}</td>
