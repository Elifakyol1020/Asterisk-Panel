...
public class ContextTenantResolver implements TenantResolver {
    ...
    private static final Pattern NUMBER_CHANNEL = Pattern.compile("^(?:PJSIP|SIP)/[0-9]{1,20}-([0-9]{4,6})(?:-[a-zA-Z0-9]+)?$");
    public Long resolveTenant(CdrInput input) {
        ...
        extractNumber(input.channel(), candidates);
        extractNumber(input.dstChannel(), candidates);
        if (candidates.size() != 1) throw rejected();
        Long id = candidates.iterator().next();
        if (!tenants.existsById(id)) throw rejected();
        return id;
    }
    private void extractNumber(String value, Set<Long> candidates) {
        if (value == null) return;
        var match = NUMBER_CHANNEL.matcher(value.trim());
        if (match.matches()) candidates.add(tenants.findByCode(match.group(1)).orElseThrow(this::rejected).getId());
    }
