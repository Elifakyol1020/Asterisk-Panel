...
import com.netgsm.asterisk.mapper.UserMapper;
...
public class UserService {
    ...
    public Page<UserResponse> list(Long tenantId, Pageable page) {
        ...
        return repository.findAllByTenantId(tenantId, page).map(mapper::toResponse);
    }
    ...
    public UserResponse create(Long tenantId, CreateUserRequest request) {
        ...
        User user = mapper.toEntity(request, username, email);
        user.setTenantId(tenantId);
        user.setRole(Role.TENANT_ADMIN);
        user.setPasswordHash(hash(request.password()));
        repository.saveAndFlush(user);
        log.info("User created id={} tenantId={}", user.getId(), tenantId);
    ...
    public UserResponse update(Long id, UpdateUserRequest request) {
        ...
        mapper.update(request, user, username, email);
    ...
    private User find(Long id) {
        return repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("User"));
    }
