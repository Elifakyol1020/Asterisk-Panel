...
class InboundRoutingTest {
 @Test void incomingRouteValidatesTrunkAndTargetWithinTheAuthenticatedTenant() {
  ...
  var service=new InboundRouteService(repository,current,refs,provisioning,new AsteriskNaming());
 ...
 @Test void provisionedIncomingNumberDoesNotOverwriteInternalExtension() {
  ...
  var service=new AsteriskDialplanProvisioningService(new AsteriskNaming(),mock(InboundRouteRepository.class),realtime,endpoints,mock(QueueRepository.class),mock(IvrRepository.class),mock(TrunkRepository.class),mock(ExtensionRepository.class),mock(IvrOptionRepository.class),writer);
  ...
  assertEquals("PJSIP/tenant1_1001,30",rows.getAllValues().getFirst().getAppdata());
